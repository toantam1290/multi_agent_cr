...
def interpret_funding(funding_rate: float, direction: str) -> tuple[float, str]:
    ...
    Returns: (point_adjustment, reason)
    ...
    if fr_pct > 0.10:
        if direction == "SHORT":
            return 10, f"funding extreme +{fr_pct:.3f}% → market overleveraged long, SHORT boost"
        return -12, f"funding extreme +{fr_pct:.3f}% → LONG risky, crowd too long"

    # Funding elevated dương
    if fr_pct > 0.05:
        if direction == "SHORT":
            return 5, f"funding elevated +{fr_pct:.3f}% → SHORT confluence"
        return -5, f"funding elevated +{fr_pct:.3f}% → LONG caution"

    # Funding extreme âm → thị trường overleveraged short
    if fr_pct < -0.07:
        if direction == "LONG":
            return 10, f"funding extreme {fr_pct:.3f}% → market overleveraged short, LONG boost"
        return -12, f"funding extreme {fr_pct:.3f}% → SHORT risky, crowd too short"

    # Funding elevated âm
    if fr_pct < -0.03:
        if direction == "LONG":
            return 5, f"funding negative {fr_pct:.3f}% → LONG confluence"
        return -5, f"funding negative {fr_pct:.3f}% → SHORT caution"

    # Neutral zone
    return 0, f"funding neutral {fr_pct:.3f}%"
...
def interpret_oi(
    ...
    if oi_up and price_up:
        if direction == "LONG":
            return 7, f"OI+{oi_change_pct:.1f}% + price up → real long buildup, LONG confirmed"
        return -5, f"OI+{oi_change_pct:.1f}% + price up → long trend building, SHORT risky"

    # Tình huống 2: OI tăng + price giảm → shorts vào thật → xác nhận SHORT SMC
    if oi_up and not price_up:
        if direction == "SHORT":
            return 7, f"OI+{oi_change_pct:.1f}% + price down → real short buildup, SHORT confirmed"
        return -5, f"OI+{oi_change_pct:.1f}% + price down → short trend building, LONG risky"

    # Tình huống 3: OI giảm + price tăng → short squeeze hoặc long unwind đảo chiều
    if oi_down and price_up:
        if direction == "LONG":
            return 5, f"OI{oi_change_pct:.1f}% + price up → short squeeze, LONG momentum"
        return -8, f"OI{oi_change_pct:.1f}% + price up → short squeeze ongoing, SHORT dangerous"

    # Tình huống 4: OI giảm + price giảm → long liquidation
    if oi_down and not price_up:
        if direction == "SHORT":
            return 5, f"OI{oi_change_pct:.1f}% + price down → long liq, SHORT momentum"
        return -8, f"OI{oi_change_pct:.1f}% + price down → long liquidation, LONG dangerous"

    # OI thay đổi nhỏ — không đủ signal
    return 0, f"OI change {oi_change_pct:.1f}% — neutral"
...
def interpret_cvd(
    ...
    if not in_zone:
        if direction == "LONG" and cvd_trend == "accelerating_buy":
            return 3, f"CVD accelerating buy (ratio={cvd_ratio:.2f}), LONG momentum"
        if direction == "SHORT" and cvd_trend == "accelerating_sell":
            return 3, f"CVD accelerating sell (ratio={cvd_ratio:.2f}), SHORT momentum"
        return 0, f"CVD ratio={cvd_ratio:.2f} (not in SMC zone)"

    # Giá đang ở trong OB/FVG zone → CVD là confirmation cực quan trọng
    if direction == "LONG":
        if cvd_ratio > 0.58 or cvd_trend == "accelerating_buy":
            return 8, f"CVD confirms LONG at OB/FVG zone (ratio={cvd_ratio:.2f}, {cvd_trend})"
        if cvd_ratio < 0.42 or cvd_trend == "accelerating_sell":
            return -10, f"CVD contradicts LONG — sellers in zone (ratio={cvd_ratio:.2f})"
        return 2, f"CVD neutral at zone (ratio={cvd_ratio:.2f})"

    if direction == "SHORT":
        if cvd_ratio < 0.42 or cvd_trend == "accelerating_sell":
            return 8, f"CVD confirms SHORT at OB/FVG zone (ratio={cvd_ratio:.2f}, {cvd_trend})"
        if cvd_ratio > 0.58 or cvd_trend == "accelerating_buy":
            return -10, f"CVD contradicts SHORT — buyers in zone (ratio={cvd_ratio:.2f})"
        return 2, f"CVD neutral at zone (ratio={cvd_ratio:.2f})"

    return 0, "CVD: direction unknown"
